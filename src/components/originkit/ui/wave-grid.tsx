 ...
 *
 * Event listeners are attached to `window` rather than the canvas so that
 * ripples respond to pointer movement anywhere on the page, even when the
 * canvas sits beneath a z-indexed UI layer with pointer-events blocked.
 */
...
class WaveGridScene {
    ...
    constructor(container: HTMLElement, cfg: Config) {
        ...
        // No cursor — pointer events come from window, not this canvas
    ...
    /** Maps a window-level PointerEvent onto the y=0 world plane via the canvas rect. */
    ...
    /**
     * Binds to `window` instead of the canvas so ripples fire regardless of
     * which element is hit-tested on top. The canvas rect is still used to map
     * client coordinates into NDC.
     */
    private bindEvents() {
...
function WaveGridBase(props: WaveGridProps) {
    const {
        base = DEFAULTS.base,
    ...
    const cfgRef = useRef<Config>({
        base, crest, grid, gap, amplitude, waveSpeed, trail, tilt, shadows, sizePercent,
    })
    cfgRef.current = { base, crest, grid, gap, amplitude, waveSpeed, trail, tilt, shadows, sizePercent }
    ...
    return (
        <div
            ref={containerRef}
            role="img"
            aria-label="Interactive wave grid background"
            style={{
                position: "relative",
                width: "100%",
                height: "100%",
...
const presetProps: WaveGridProps = {
    base: "#FFFFFF",
    crest: "#00FFFF",
    grid: 20,
    gap: 2,
...
export default function WaveGrid(props: WaveGridProps) {
    return <WaveGridBase {...presetProps} {...props} />
}
